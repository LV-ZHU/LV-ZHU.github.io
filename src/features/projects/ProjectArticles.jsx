...
function useArchSvg(mapRef) {
  useEffect(() => {
    ...
    const debouncedDraw = () => { clearTimeout(resize_timer); resize_timer = setTimeout(draw, 150) }
                ...
                <div className="code-block">{`static bool is_ipaddr_valid(const char* const ipstr)
{
  ...
  soulter/astrbot:latest`}</div>
            ...
            <p>创建服务文件在 <code> /etc/systemd/system/napcat.service </code>目录下，具体的诸如 xvfb-run 的路径可以用<code> which </code>指令查询后替换：</p>
...
WantedBy=multi-user.target`}</div>
            ...
            {/* ====== 节省服务器内存 ====== */}
            <h3 className="topic-title">节省服务器内存</h3>
            <p>如果用的是小规格服务器，跑 AstrBot+NapCat+QQ 资源可能会比较吃紧，容易夯住然后磁盘 IO 读写受限。</p>

            <h4 className="topic-title">内存占比排查方式</h4>
            <div className="issue-card">
              <p>几个常用的排查命令：</p>
              <div className="cmd-inline">
            ...
            <h4 className="topic-title">使用 Swap </h4>
            <div className="issue-card">
...
echo '/swapfile none swap sw 0 0' | sudo tee -a /etc/fstab`}</div>
            ...
            <h4 className="topic-title">清理 QQ 缓存 </h4>
            <div className="issue-card">
...
rm -f ~/.config/QQ/*.log`}</div>
            ...
            <h4 className="topic-title">关掉不需要的系统服务</h4>
            <div className="issue-card">
...
sudo systemctl disable snapd snapd.socket`}</div>
            ...
            <h4 className="topic-title">NapCat 日志清理</h4>
            <div className="issue-card">
